...
def getMovieAttractions(predictions, userId):
    ...
    hist_usr = train_df[train_df.userId == userId].sort_values("rating", ascending=False).merge(movies_df, on='movieId')
...
def analyze_click_probabilities(data):
    print("Independent t-test:")
    t_statistic, p_value = stats.ttest_ind(data['cp_carousel'], data['cp_list'])
    print("t-statistic = %g, p-value = %g" % (t_statistic, p_value))

    w_statistic, p_value = stats.levene(data['cp_carousel'], data['cp_list'])
    print("W-statistic = %g, p-value = %g" % (w_statistic, p_value))

#example use
t100 = pd.read_csv('files/cp_compare_limit_trn_top100_new.csv')
analyze_click_probabilities(t100)
